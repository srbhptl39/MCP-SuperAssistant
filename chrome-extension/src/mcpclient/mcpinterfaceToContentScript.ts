...
class McpInterface {
  ...
  private async handleGetToolDetails(connectionId: string, message: any): Promise<void> {
    ...
    console.log(`[MCP Interface] Getting available tools for request ${requestId}`);

    try {
      // Get the primitives from the server using the persistent connection
      const primitives = await this.getAvailableToolsFromServer();
      ...
      console.log(`[MCP Interface] Tool details request ${requestId} completed successfully`);
    ...
    try {
      ...
      if (isConnected) {
        ...
        try {
          ...
          const freshTools = await this.getAvailableToolsFromServer();
   ...
   */
  private async getAvailableToolsFromServer(): Promise<Primitive[]> {
    try {
      console.log('[MCP Interface] Getting available primitives from server');

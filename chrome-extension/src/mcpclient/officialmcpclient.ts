...
async function isServerAvailable(url: string): Promise<boolean> {
  try {
    ...
    // If we get here, the server is available (though we can't check status in no-cors mode)
    console.log(`Server at ${url} appears to be available`);
    return true;
  } catch (error) {
    console.error(`Server at ${url} is not available:`, error);
    return false;
  }
}
 ...
 */
export async function getPrimitivesWithSSE(uri: string): Promise<Primitive[]> {
  try {
    // Connect to the server if not already connected
    await persistentClient.connect(uri);

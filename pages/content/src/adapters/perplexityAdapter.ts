...
import {
  ...
  createPerplexityPatternObserver,
  insertToolResultToChatInput,
} from '../components/websites/perplexity';
...
export class PerplexityAdapter extends BaseAdapter {
  ...
  private patternObserver: PerplexityPatternUnifiedObserver | null = null;
  ...
  protected initializeSidebarManager(): void {
    // Create a tool output handler for Perplexity
    const perplexityToolOutputHandler: ToolOutputHandler = {
      getProcessedToolOutputs,
      getMcpToolContents,
      insertToolResultToChatInput,
    };

    this.sidebarManager = SidebarManager.getInstance('perplexity', perplexityToolOutputHandler);
    this.sidebarManager.initialize();
  }

  /**
   * Get elements that might contain tool commands in Perplexity
  ...
  getToolCommandElements(root: Element = document.body): Element[] {
    ...
    const markdownContainers = Array.from(root.querySelectorAll('[id^="markdown-content-"]'));
    ...
    if (toolCommandElements.length > 0) {
      logMessage(`Found ${toolCommandElements.length} potential tool command elements in Perplexity`);
    }
  ...
  protected initializeObserver(forceReset: boolean = false): void {
    ...
    if (forceReset && this.patternObserver) {
      ...
      logMessage('Forced reset of pattern-based observer for Perplexity');
    }
    ...
    if (!this.patternObserver) {
      ...
      this.patternObserver = createPerplexityPatternObserver(this, perplexityToolOutputHandler);
      logMessage('Created new pattern-based observer for Perplexity');
    }

    // Start observing with the pattern-based observer
    this.patternObserver.observeAllElements();
    logMessage('Pattern-based observer started for Perplexity');
  ...
  /**
   * Set up a check for navigation events in Perplexity
  ...
  private setupNavigationCheck(): void {
    ...
    this.urlCheckInterval = window.setInterval(() => {
      ...
      if (this.lastUrl !== currentUrl) {
        ...
        logMessage('Detected URL change in Perplexity, re-initializing observer');
  ...
  private handlePopState = (): void => {
    ...
    if (this.lastUrl !== currentUrl) {
      this.lastUrl = currentUrl;
      logMessage('Detected popstate navigation in Perplexity, re-initializing observer');
      this.initializeObserver(true);
    }
  };
  ...
  private handleMcpToolDetected(event: CustomEvent<{tool: DetectedTool; domPosition?: number}>): void {
    if (event.detail && event.detail.tool) {
      ...
      if (!exists) {
        ...
        logMessage(`Added new detected tool: ${tool.name} with signature ${newToolSignature} at position ${domPosition}`);
      } else {
        logMessage(`Skipped duplicate tool: ${tool.name} with signature ${newToolSignature} at position ${domPosition}`);
      }
    }
  }
  ...
  triggerSubmission(): void {
    ...
    submitChatInput().then((success: boolean) => {

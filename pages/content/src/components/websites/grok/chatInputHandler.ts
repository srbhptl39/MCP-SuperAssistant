...
/**
 * Find the Grok chat input element
 * @returns The chat input element or null if not found
 */
export const findChatInputElement = (): HTMLElement | null => {
  // Try to find Grok's textarea or contenteditable div
  // Note: These selectors may need to be updated based on Grok's actual DOM structure
  ...
  if (grokInput) {
    logMessage('Found Grok input element');
    lastFoundInputElement = grokInput as HTMLElement;
    return grokInput as HTMLElement;
  }
  ...
  for (const selector of possibleInputSelectors) {
    ...
    if (element) {
      logMessage(`Found Grok input element via selector: ${selector}`);
  ...
  logMessage('Could not find Grok input element');
  return null;
};
...
export const insertTextToChatInput = (text: string): boolean => {
  try {
    ...
    if (!chatInput) {
      logMessage('Could not find Grok input element');
      console.error('Could not find Grok input element');
      return false;
    }
    ...
    logMessage(`Error inserting text into Grok input: ${error}`);
...
export const insertToolResultToChatInput = (result: string): boolean => {
  logMessage('Inserting tool result to Grok chat input');
...
export const submitChatInput = async (maxWaitTime = 5000): Promise<boolean> => {
  try {
    ...
    logMessage(`Error submitting Grok chat input: ${error}`);
